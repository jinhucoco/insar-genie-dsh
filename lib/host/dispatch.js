...
export const DISPATCH_FORM = "instructions";
/** 领域关键词：命中即认为用户在问 SBAS-InSAR 这件事 */
const DOMAIN_RE = /insar|in-sar|sbas|sarscape|sentinel|哨兵|slc|gacos|poeorb|ztd|干涉|形变|沉降|解缠|连接图|反演|地理编码|基线|时相|多视|实验|earthdata/i;
/**
 ...
 */
export function detectDispatch(text) {
...
let injectionSeq = 0;
/** 构造一条调度指令注入消息 */
export function buildDispatchMessage(turn) {
    injectionSeq += 1;
    return {
 ...
 */
export function registerDispatch(ctx) {
    const host = ctx;
    let warned = false;
    ...
    host.on("agent/pre-step", async (payload, next) => {
        ...
        try {
            ...
            if (payload?.step !== 1)
                return decision;
            if (!collectUserTexts(payload.messages).some(detectDispatch))
                return decision;
            const message = buildDispatchMessage(payload.turn ?? 0);
            return { ...decision, messages: [...(decision.messages ?? []), message] };
        }
        catch (error) {
            warnOnce(`insar-genie: 关键词调度注入失败，已跳过：${String(error?.message ?? error)}`);
            return decision;
        }
    }, { prepend: true });
}
