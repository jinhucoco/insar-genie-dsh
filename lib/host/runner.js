 ...
 *  传入正数才启用超时。 */
export function runPython(pythonBin, args, cwd, timeoutMs) {
    return new Promise((resolve) => {
        const child = spawn(pythonBin, args, { cwd, stdio: ["ignore", "pipe", "pipe"] });
        let stdout = "";
        let stderr = "";
        const capped = (s, chunk) => {
            const next = s + chunk.toString("utf8");
            return next.length > OUTPUT_CAP ? next.slice(-OUTPUT_CAP) : next;
        };
        child.stdout?.on("data", (b) => { stdout = capped(stdout, b); });
        child.stderr?.on("data", (b) => { stderr = capped(stderr, b); });
