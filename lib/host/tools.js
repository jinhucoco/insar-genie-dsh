...
function slcDataDir(s, exp) {
    if (exp.dataDirs?.slc)
        return exp.dataDirs.slc;
    if (s?.workDir)
        return join(s.workDir, "slc");
    return "";
}
 ...
 */
function detectSuperReference(slcDir) {
    if (!slcDir || !existsSync(slcDir))
        return "";
    try {
        ...
        const plain = readdirSync(slcDir)
            ...
            .sort();
        if (plain.length > 0)
            return join(slcDir, plain[Math.floor(plain.length / 2)]);
    }
    catch {
        /* 目录读失败 -> 返回空走 bat 兜底 */
    }
