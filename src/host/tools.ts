...
import { checkConnectionGraph, checkParamsConsistency, buildParamsSnapshot } from "./pipeline.js";
...
) => Promise<{ ok: boolean; step: string; [k: string]: unknown }>;

/** 各步 → PARAMETERS_INFO_<MODULE>_CMD_*.xml 的模块匹配段（latestParamsInfo 用 includes(moduleKey)）。 */
export const STEP_MODULE_KEY: Record<string, string> = {
...
export function registerTools(
  ...
  ctx.tools.register(defineTool({
    name: "insar_pipeline",
    description: "Run the full SBAS processing pipeline for an experiment: generate config.env, then execute connection graph (with an automatic baseline-widening gate), interferogram+unwrapping, inversion step1, inversion step2, geocoding. Every step verifies the run parameters match the confirmed snapshot (PARAMETERS_INFO consistency gate; aborts on mismatch unless ignoreInconsistency). Requires ENVI/SARscape on the machine.",
    parameters: {
      experimentId: { type: "string", required: true, description: "Experiment id from the registry." },
    ...
    output: JSON_OUTPUT,
    async execute(input: { experimentId: string; confirmMode?: string; ignoreInconsistency?: boolean }) {
      const exp = deps.registry.get(input.experimentId);
      if (!exp) throw new Error(`experiment not found: ${input.experimentId}`);
      const s = deps.settings?.get();
      if (!s) throw new Error("insar_pipeline: settings not configured (ASF/ENVI/SARscape paths)");

      // ① config.env：写到 bat 读取的 experimentRoot（bat 用 %~dp0..\\..\config.env，上两级 = experimentRoot）。
      const experimentRoot = resolveExperimentDir(exp.dir);
      const tmpDir = join(exp.dir, "tmp");
      ...
      writeConfigEnv(experimentRoot, envInput);
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        await runStep(exp, "cg", { maxPercBaseline });
        cgCheck = checkConnectionGraph(exp.dir);
      ...
      const stepResults: { step: string; ok: boolean; unverified?: string[]; missingInfo?: boolean }[] = [
        { step: "cg", ok: cgCheck.passed },
      ];
      for (const { step, moduleKey } of steps) {
        await runStep(exp, step);
