...
// 引入 dsh-skill 的类型合并（ctx.skills 的 declare module 声明），使 skill 注册类型可见
...
export function registerSkill(ctx: Context): void {
  ...
  ctx.skills.register({
    ...
    source: "runtime",
    description:
      "SBAS-InSAR 全链路 AI 技能：从 Sentinel-1 SLC 数据下载、配套数据（DEM/GACOS/POEORB）获取，到 SARscape 实验参数确认与批处理执行，再到守护监控，全程 AI 与用户对话交互、AI 自动执行。只要用户提到 insar、SBAS、实验、哨兵、Sentinel-1、干涉、形变、沉降、SARscape、GACOS、POEORB、DEM 等领域关键词就会触发；用户没给出明确任务时，先列插件功能菜单问用户想做哪一件（下载SLC主数据/下载配套数据/SLC批量导入/参数确认+跑SBAS全流程/单步执行/查实验进展/环境自检与账号配置/DEM预处理）。",
    whenToUse:
      "用户提到 insar / SBAS / 实验 / 哨兵 / Sentinel-1 / 干涉 / 形变 / 沉降 / SARscape / GACOS / POEORB / DEM 等领域关键词（哪怕只是一个裸关键词），或要从 ASF 下载 Sentinel-1 SLC、开展 SBAS-InSAR 实验（下载→配套数据→参数确认→批处理→监控）、批量导入 SLC、查实验进展、配置环境与账号、预处理 DEM。",
    content,
