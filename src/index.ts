...
export function registerSkill(ctx: Context): void {
  ...
  ctx.skills.register({
    ...
    source: "runtime",
    description:
      "SBAS-InSAR 全链路 AI 技能：从 Sentinel-1 数据下载、配套数据（DEM/GACOS/POEORB）获取，到 SARscape 实验参数确认与批处理执行，再到守护监控，全程 AI 与用户对话交互、AI 自动执行。用户只需说出需求（提供时间范围 + shp/kml 矢量 + 极化），AI 自动完成。触发词：“从ASF下载哨兵数据”、“下载Sentinel-1”、“ASF下载S1”、“开始SBAS实验”、“跑SBAS”、“参数怎么设”、“实验进展如何”。",
